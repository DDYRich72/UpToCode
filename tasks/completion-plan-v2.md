# UpToCode Completion Plan v2 — Competition-First, Always Submission-Ready
...
python -m ruff check uptocode tests scripts
python -m mypy uptocode
...
The entire production build (~1,560 changed lines, 27 untracked paths) is uncommitted on `codex/uptocode-build`.

- [ ] Verify ignores (`web/node_modules/`, `web/dist/`, `web/.wrangler/`; `web/.vinext/` only if regenerated by build; `.uptocode/` outputs stay ignored). Move `web/build/sites-vite-plugin.ts` to a tracked path (e.g. `web/plugins/`) now so nothing tracked lives under an ignored directory.
...
- [ ] README: add the required **"Built with Codex"** section — how Codex was used throughout (gates, FIXPLAN hand-offs, this plan), plus how GPT-5.6 powers the judgment tier; add exact judge test-build instructions (clone → install → `uptocode scan fixtures/bad_python` → review → plan → `serve`). Optional, operator's choice: one truthful sentence noting that independent AI code review produced the work orders Codex executed (the rules require no such disclosure; the planning documents are in the repo either way).
...
- [ ] Hosted minimal controls: in-memory per-key rate limit (env-tunable, default 30/min, `429` + `Retry-After`), `UPTOCODE_HOSTED_JUDGMENT` gate default **off**, safe key-prefix in logs. (This replaces the Firestore plane for 1.0.)
...
- [ ] Self-scan story sharpened for the demo: `scripts/compliance.py` zero-findings evidence is a headline beat ("UpToCode audits itself").
...
PyPI name recheck → trusted publishing → tag-driven release with SBOM/provenance → public
repo decision → MCP registry submission with final `server.json` → separately authorized
post-deploy live GPT-5.6 smoke → `specs/004-beta-access-control.md` and
`specs/005-typescript-analysis.md` as unimplemented 1.1 specifications.
