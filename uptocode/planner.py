...
from uptocode.models import Finding, Report, ReviewManifest
...
def _entry(finding: Finding) -> str:
    ...
    checks = [
        f"UpToCode no longer emits {finding.rule_id} at this location.",
...
def generate_fixplan(report: Report, manifest: ReviewManifest) -> str:
    ...
    sections = ["# UpToCode FIXPLAN", "", "Generated from explicitly approved findings.", ""]
    if not findings:
