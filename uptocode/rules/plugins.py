...
from uptocode.models import AnalysisWarning, Finding, StrictModel
...
def _load_path(path: Path) -> RulePlugin:
    ...
    spec = importlib.util.spec_from_file_location(
        f"uptocode_external_{abs(hash(str(resolved)))}", resolved
    )
...
def load_rule_plugins(entries: list[str], *, root: Path) -> list[RulePlugin]:
    ...
    installed = {entry.name: entry for entry in importlib.metadata.entry_points(group="uptocode.rules")}
