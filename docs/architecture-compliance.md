...
| AA002 | Typed policies cap file, aggregate source, request, output, rule-call, elapsed-time, and hosted concurrency usage. | `ScanConfig`, `JudgmentConfig`, MCP byte limits, Cloud Run concurrency | `test_gate2_edges.py`, `test_mcp_server.py`, `test_production_contract.py` |
...
| AA004 | Configuration, paths, diffs, schemas, MCP requests, and reports are strictly validated. | strict Pydantic models, diff reconstructor, canonical containment, Draft 2020-12 validator | `test_production_contract.py`, `test_gate2_edges.py` |
...
| AA009 | MCP tools have purpose-specific strict schemas, descriptions, structured outputs, annotations, and actionable errors. | FastMCP typed tools and rule resource | `test_mcp_server.py`, `test_hosted_lifecycle.py` |
...
| AA012 | Payload-free timing, counts, error classes, model usage, versions, and correlation-capable MCP requests exist at boundaries. | Report metadata/usage, MCP lifecycle logs, operations policy | `test_gate1.py`, `test_judgment.py`, `test_hosted_lifecycle.py` |
...
finding, suppression, or unexplained warning. The release workflow includes that
