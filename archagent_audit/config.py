...
from archagent_audit.models import Severity, SkippedFile, StrictModel
...
def _environment_overrides(config: ScanConfig) -> ScanConfig:
    ...
    if model := os.getenv("ARCHAGENT_JUDGMENT_MODEL"):
        values["model"] = model
    if base_url := os.getenv("ARCHAGENT_OPENAI_BASE_URL"):
        values["base_url"] = base_url
    numeric = {
        "ARCHAGENT_JUDGMENT_TIMEOUT": ("timeout_seconds", float),
        "ARCHAGENT_JUDGMENT_MAX_OUTPUT_TOKENS": ("max_output_tokens", int),
        "ARCHAGENT_JUDGMENT_RULE_BUDGET": ("rule_call_budget", int),
        "ARCHAGENT_JUDGMENT_MAX_RETRIES": ("max_retries", int),
    }
    try:
        ...
        if raw_size := os.getenv("ARCHAGENT_MAX_FILE_SIZE"):
            root_values["max_file_size"] = int(raw_size)
        if raw_deadline := os.getenv("ARCHAGENT_DEADLINE_SECONDS"):
            root_values["deadline_seconds"] = float(raw_deadline)
        return ScanConfig.model_validate(root_values)
    except (TypeError, ValueError, ValidationError) as error:
        raise ValueError(f"Invalid ArchAgent environment configuration: {error}") from error
